...
// ///1*********perfect version just updated UI after
// import React, { useState, useEffect } from 'react';
...
const EditPortfolio = () => {
  ...
  const [updateMessage, setUpdateMessage] = useState('');
  const [messageType, setMessageType] = useState('');
  const [errors, setErrors] = useState({}); // ✅ validation tracking
  ...
  const handleSubmit = async (e) => {
    ...
    const newErrors = {};
    if (!portfolioData.title.trim()) newErrors.title = 'Title is required';
    if (!portfolioData.unit.number) newErrors.unit = 'Unit is required';
    if (!portfolioData.postcode.trim()) newErrors.postcode = 'Postcode is required';
    if (!portfolioData.learningOutcome.number)
      newErrors.learningOutcome = 'Learning Outcome is required';
    if (!portfolioData.criteria?.number)
      newErrors.criteria = 'Criteria is required';
    if (!portfolioData.method) newErrors.method = 'Method is required';
        ...
        <Card className="mb-4 shadow-sm">
          ...
          <Card.Body>
            ...
            <Row>
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  />
                  {errors.postcode && (
                    <Form.Text className="text-danger">{errors.postcode}</Form.Text>
                  )}
                </Form.Group>
              </Col>
            </Row>
            ...
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Select
                    ...
                    isInvalid={!!errors.learningOutcome}
                  >
                    <option value="">Select Learning Outcome</option>
                    {getLearningOutcomes(portfolioData.unit.number).map((lo) => (
                      <option key={lo.LO_number} value={lo.LO_number}>
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  {errors.criteria && (
                    <Form.Text className="text-danger">{errors.criteria}</Form.Text>
                  )}
                </Form.Group>
              </Col>
            </Row>
            ...
            <Form.Group className="mb-3">
              ...
              {errors.method && (
                <Form.Text className="text-danger">{errors.method}</Form.Text>
              )}
            </Form.Group>
          </Card.Body>
        </Card>

        {/* Task Details + Images Sections — remain unchanged */}
        {/* (Your existing cards stay the same here) */}
